-- Adiciona coluna locked_by_action para evitar concorrência em atualizações de status sensíveis
ALTER TABLE public.agent_jobs ADD COLUMN IF NOT EXISTS last_action_id UUID DEFAULT NULL;

-- Função para executar ações de job com segurança (atômica)
...
BEGIN
    ...
    -- Define novo status e valida transição
    CASE p_action
        WHEN 'cancel' THEN
            IF v_job.status IN ('completed', 'failed') THEN
                RETURN jsonb_build_object('ok', false, 'error', 'Job already finished');
            END IF;
            v_new_status := 'failed';
        WHEN 'pause' THEN
    ...
    VALUES (p_job_id, p_action, p_actor_id, p_correlation_id, jsonb_build_object('source', 'rpc_action', 'timestamp', now()))
