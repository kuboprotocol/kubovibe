...
import { Plus, FileText, Trash2, LogOut, Code, Pencil, UserCircle, Sparkles, Search, MoreHorizontal } from 'lucide-react'
    ...
    <div className="min-h-screen bg-background relative">
      ...
      <main className="max-w-6xl mx-auto px-6 py-10 relative z-10">
        ...
        {loading ? (
          ...
          >
            <div className="glass glass-border rounded-3xl p-12 max-w-md mx-auto shadow-glow-lg">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map((project, index) => (
              ...
              >
                <div
                  className="group cursor-pointer glass glass-border rounded-2xl p-5 hover:shadow-glow transition-all duration-300 hover:border-primary/30"
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-8 w-8 rounded-lg opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-foreground transition-all"
      ...
      <Dialog open={!!renameProject} onOpenChange={(open) => !open && setRenameProject(null)}>
        <DialogContent className="glass rounded-2xl" onClick={(e) => e.stopPropagation()}>
          <DialogHeader>
            <DialogTitle className="font-display">Renomear projeto</DialogTitle>
          </DialogHeader>
          <Input
            value={newTitle}
