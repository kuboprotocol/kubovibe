...
cmd_test_parser() {
  ...
  printf '%s\n' \
    ...
    '- 🌱 **Seeds executed (last 50 runs):** [`fc-seedz.json`](https://x/seeds)' \
  ...
  {
    printf 'header\n'
    printf -- '- 🌱 **Seeds executed (last 50 runs):** [`fc-seedz.json`](https://x/seeds)\r\r\n'
  ...
  printf '%s\n' \
    '- 🌱 __Seeds executed (last 50 runs):__ [`fc-seedz.json`](https://x/seeds)' \
  ...
  # 6) Fallback markers
  printf '%s\n' \
    '- 🌱 `fc-seedz.json` — _(not produced in this run)_' \
  ...
  printf '%s\n' \
    '- 🌱 **Seeds executed (last 50 runs):** [`fc-seedz.json`](https://wrong/seeds)' \
    '- 💥 **Minimized counterexamples (last 100):** [`fc-failures.json`](https://x/failures)' \
    > "${tmp}/wrongurl.md"
  ...
  printf '%s\n' \
    '- 🌱 **Seed executed (last 50 runs):** [`fc-seedz.json`](https://x/seeds)' \
  ...
  printf '%s\n' \
    '- 🌱 `fc-seedz.json` — (not produced in this run)' \
  ...
  printf '%s\n' \
    '- 🌱 **Seeds executed (last 50 runs):** [`fc-seedz.json`](https://x/seeds) extra-text' \
